...
class Siswa extends React.Component {
   ...
   return header
    ...
    handleSave = (event) => {
        ...
        /* menampung data id_siswa,nis, nama_siswa, kelas, jurusan dan poin dari Form ke dalam FormData() untuk dikirim  */
        let url = "";
        if (this.state.action === "insert") {
          url = "http://localhost:2200/siswa/save"
        } else {
          url = "http://localhost:2200/siswa/update"
        }
        let form = {
    ...
    getJurusan = () => {
      let url = "http://localhost:2200/siswa/jurusan";
      // mengakses api untuk mengambil data siswa
      axios.get(url, this.headerConfig())
      .then(response => {
        // mengisikan data dari respon API ke array siswa
    ...
    Drop = (id_siswa) => {
        let url = "http://localhost:2200/siswa/" + id_siswa;
        ...
        return(
            ...
            <Card>
                ...
                <Modal show={this.state.isModalOpen} onHide={this.handleClose}>
                    ...
                    Kelas
                    <select name="kelas"  value={this.state.kelas} onChange={this.bind} className="form-control" required>
                      <option disabled hidden value=''>--Pilih Kelas--</option>
                    ...
                    <option disabled hidden value=''>--Pilih Jurusan--</option>
